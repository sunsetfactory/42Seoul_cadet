...
void Command::run(int fd)
{
	...
	// 문자열이 비어있지 않고, 마지막 두 문자가 "\r\n"이 아닌 경우
	{	
		std::string msg = serverMsg.str();
		std::cout << "#msg: " << msg << std::endl;
		if (!msg.empty() && msg[msg.size() - 1] != '\n')
		{
			std::string str;
			serverMsg >> str;
			clientList[fd].eofBuffer << str;
			clientList[fd].eofFlag = true;
			return ;
		}
	}
	...
	{	// irssi가 아닌 경우
		...
		std::string eofBuffer;
		if (clientList[fd].eofFlag == true )
		{
			eofBuffer = clientList[fd].eofBuffer.str();
			serverMsg << eofBuffer;
		}
		while (getline(serverMsg, cmdBuffer, ' '))	// 명령어 파싱
		{
			// if (eofBuffer.empty() == false)
			// {
			// 	cmdBuffer = eofBuffer + cmdBuffer;
			// 	eofBuffer.clear();
			// }
			// std::cout << "#WHILE IN cmdBuffer " << cmdBuffer << std::endl;
			// std::cout << "#WHILE IN cmdBuffer " << cmdBuffer << "||" << std::endl;

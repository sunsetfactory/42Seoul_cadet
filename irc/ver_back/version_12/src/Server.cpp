...
void Server::execute()
{
	...
	while (1)
	{
		...
		{
			logFile << "-----------------" << std::endl;
			logFile << "event_ident: " << _eventList[i].ident << std::endl;
			logFile << "event_filter: " << _eventList[i].filter << std::endl;
			logFile << "event_data: " << _eventList[i].data << std::endl;
			logFile << "-----------------" << std::endl;
			std::cout << "#eventCnt : " << _eventCnt << std::endl;
		...
		{
			if (iter->second.eofFlag == false && iter->second.getReciveBuf().empty() == false)
			{

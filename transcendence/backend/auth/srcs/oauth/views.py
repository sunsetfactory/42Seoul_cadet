...
redirect_uri = "http://10.19.212.254:8002/oauth/login/redirect"
...
def login_redirect(request):
    try:
        ...
        if Auth.objects.filter(username=username, is_active=True).exists():
            ...
            return HttpResponseRedirect(f"https://10.19.212.254/#/login/2fa?username={username}")
        # Auth 테이블에 유저가 없는 경우 회원가입
            ...
            # 유효성 검사에서 통과하면 회원가입            
            serializer.save()
            return HttpResponseRedirect("https://10.19.212.254")

...
- 프로그램은 역폴란드 표기법(후위 표기법)[^ko1]을 인자로 사용해야 합니다.
  [^ko1]: https://ko.wikipedia.org/wiki/%EC%97%AD%ED%8F%B4%EB%9E%80%EB%93%9C_%ED%91%9C%EA%B8%B0%EB%B2%95
...
- 프로그램은 정수 수열을 인자로 받아야 합니다.
- Your program must use the merge-insert sort algorithm to sort the positive integer sequence.
- 프로그램은 합병-삽입 정렬 알고리즘을 이용하여 정수 수열을 정렬하여야 합니다.

> [!CAUTION]
> To clarify, yes, you need to use the Ford-Johnson algorithm.

> [!CAUTION]
> 명확하게, Ford-Johnson 알고리즘[^2]을 사용해야 합니다.
> [^2]: https://en.wikipedia.org/wiki/Merge-insertion_sort
...
- 세번째 줄은 첫번째 컨테이너를 사용한 알고리즘으로 정수 수열을 정렬하는 데 걸린 시간을 측정해야 합니다.
...
- 마지막 줄은 두번째 컨테이너를 사용한 알고리즘으로 정수 수열을 정렬하는 데 걸린 시간을 측정해야 합니다.
...
> [!NOTE]
> 정렬을 수행하는 데 사용된 시간을 표시하는 형식은 자유지만, 선택한 정밀도는 사용된 두 컨테이너의 차이를 명확하게 볼 수 있어야 합니다.

Here is an example of a standard use:

다음은 표준 사용에 대한 예시 입니다:
...
> [!TIP]
> 이 예제에서는 시간 표시가 일부러 이상하게 되어 있습니다. 물론 정렬 부분과 데이터 관리 부분 모두 모든 작업을 수행하는 데 사용된 시간을 표시해야 합니다.

> [!CAUTION]
> Warning: The container(s) you used in the previous exercises are forbidden here.

> [!CAUTION]
> 경고: 이전 과제에서 사용된 container(들)은 여기서 사용이 금지 됩니다.

> [!NOTE]
> The management of errors related to duplicates is left to your discretion.

> [!NOTE]
> 중복과 관련된 오류 관리는 재량에 맡겨집니다.
...
평소처럼 Git 저장소에 과제를 제출합니다. 방어 기간 동안에는 저장소 내의 작업만 평가됩니다. 폴더와 파일 이름이 정확한지 다시 한 번 확인하시기 바랍니다.
